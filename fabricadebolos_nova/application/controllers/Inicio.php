...
class Inicio extends CI_Controller {
    ...
    public function editarClienteAction() {
        ...
        if(empty($this->input->post("data"))) {
            $data = "";
        } else if(empty($this->input->post("franquia"))) {
            $franquia = 1;
            $data = Data_to_Mysql($this->input->post("data"));
        } else {
            $data = Data_to_Mysql($this->input->post("data"));
        ...
        $dados = array(
            ...
            'franquia_derivadacliente' => $franquia,
            'telefone_cliente' => $this->input->post("telefone"),
            'email_cliente' => $this->input->post("email"),
            'datanascimento_cliente' => $data,
            'observacoes_cliente' => $this->input->post("obs"),
        );

        $update = $this->fabrica_model->editarCliente($dados,$this->input->post("codigo_cliente"));

        if($update) {
            echo json_encode(['retorno' => true, 'msg' => "Editado com sucesso!"]);exit;
        } else {
    ...
    public function cadastrarClienteAction() {
        ...
        if(empty($this->input->post("data")) || empty($this->input->post("franquia"))) {
            $data = "";
            $franquia = 1;
        } else {
            $data = Data_to_Mysql($this->input->post("data"));
        ...
        if($checkCPF) {
            ...
            if($insert) {
                echo json_encode(['retorno' => true, 'msg' => "Cadastrado com sucesso!", 'id' => $insert]);exit;
            } else {
                echo json_encode(['retorno' => false, 'msg' => "Erro ao cadastrar"]);exit;
            }
        } else {
    ...
    public function editarCliente() {
        ...
        if(empty($this->input->get("id"))) {
            redirect(base_url() . "inicio/clientes");exit;
        }
        
        $buscaCliente = $this->fabrica_model->buscaClienteEditar($this->input->get("id"));
        if($buscaCliente) {

            $dados['cliente'] = $buscaCliente;
            ...
            $rodape['js'] = array(
                ...
                "assets/js/funcoes.js",
                "assets/js/mascaras.js",
                "assets/js/editarCliente.js",
            );
    
            //Dados para view
            $topo['titulo'] = "Fábrica de Bolos - Editar Cliente";

            //View carrega header
            $this->load->view("sistema/header",$topo);
    
            //View carrega a pagina inicial
            $this->load->view("sistema/editarCliente",$dados);
    ...
    public function removeClienteAction() {
        if(empty($this->input->post("id"))) {
            echo json_encode(['retorno' => false, 'msg' => "Erro! Tente novamente"]);exit;
        } else {
            $remover = $this->fabrica_model->removerCliente($this->input->post("id"));

            if($remover) {
                echo json_encode(['retorno' => true, 'msg' => "Removido com sucesso"]);exit;
            } else {
                echo json_encode(['retorno' => false, 'msg' => "Erro! Tente novamente"]);exit;
            }
        }
